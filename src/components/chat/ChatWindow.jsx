...
const ChatWindow = ({ onClose }) => {
  ...
  // Guard so we only auto-send the pending message once
  ...
  // ✅ Auto-send pending message from hero input
  useEffect(() => {
    if (
      pendingMessage &&
      !hasSentPending.current &&
      ready &&
      messages.length > 0 // wait until welcome message is set
    ) {
      hasSentPending.current = true;
      clearPendingMessage();
      sendMessage(pendingMessage);
    }
  }, [pendingMessage, ready, messages.length]);
  ...
  // Core send logic extracted so both form submit and auto-send can use it
  ...
  return (
    ...
    >
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="bg-white border border-gray-200 rounded-2xl rounded-bl-none px-5 py-4 shadow-sm">
                <div className="flex gap-2">
                  <span className="w-2 h-2 bg-brand-orange/60 rounded-full animate-bounce [animation-delay:-0.3s]" />
                  <span className="w-2 h-2 bg-brand-orange/60 rounded-full animate-bounce [animation-delay:-0.15s]" />
                  <span className="w-2 h-2 bg-brand-orange/60 rounded-full animate-bounce" />
                </div>
              </div>
            </div>
          </div>
      ...
      >
        ...
        {/* Quick actions — only show on welcome screen with no pending already sent */}
        {messages.length === 1 && (
          <div className="flex gap-2 mt-3 flex-wrap">
            <button
              type="button"
              onClick={() => sendMessage(t("chatbot.actions.services"))}
            ...
            <button
              type="button"
              onClick={() => sendMessage(t("chatbot.actions.contact"))}
              className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1.5 rounded-full transition-colors"
            >
            ...
            <button
              type="button"
              onClick={() => sendMessage(t("chatbot.actions.pricing"))}
              className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1.5 rounded-full transition-colors"
            >
