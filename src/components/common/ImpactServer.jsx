...
export default function Impact({
    ...
    const additionalText = childrenArray.find((c) => c.type === Impact.AdditionalText);
    ...
    return (
        <section id="impact" className={`relative overflow-hidden ${className}`}>
            ...
            <div className="main-container">
                ...
                <div className={`mb-12 md:mb-16 lg:mb-20 ${isRTL ? "text-right" : "text-left"}`}>
                    {title && React.cloneElement(title, {
                        className: `${titleClassName} ${title.props.className || ""} ${isRTL ? "text-right md:text-right" : "text-center md:text-start"}`
                    })}
                    <div className={`${isRTL ? "text-right md:text-right" : "text-center md:text-start"}`}>
                        {additionalText}
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row items-center lg:items-start justify-between gap-8 md:gap-10 lg:gap-12">
                    ...
                    <Reveal type="slide-up" className={`w-full lg:w-1/2 ${isRTL ? "text-right" : "text-left"}`}>
                        {subtitle}
                        {description}
                    </Reveal>
