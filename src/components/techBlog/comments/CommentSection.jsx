...
import { MessageCircle, Send, User, Clock, Trash2, Edit3, X, Check, MoreVertical } from "lucide-react";
...
export default function CommentSection({ articleId, articleDocumentId, onCommentAdded }) {
  ...
  const getAvatarColor = (name = "G") => {
    const colors = ["bg-orange-500", "bg-purple-500", "bg-blue-500", "bg-emerald-500", "bg-rose-500"];
    ...
    <section id="comments" className="mt-16 pt-8 space-y-8">
      <div className="flex items-center justify-between border-b border-white/5 pb-4">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          <span className="text-white/20 text-lg font-light">({comments.length})</span>
        </h2>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="bg-white/5 p-5 md:p-6 rounded-2xl border border-white/10 backdrop-blur-md relative overflow-hidden group">
          <form onSubmit={handleSubmitComment} className="relative z-10 space-y-4">
            <div className="flex gap-3">
                ...
                <div className="flex-1 space-y-3">
                    <textarea
                        ...
                        placeholder={lang === "ar" ? "ماذا يدور في ذهنك؟" : "Share your thoughts..."}
                        className="w-full bg-transparent border-none p-0 text-white placeholder-white/20 focus:outline-none focus:ring-0 text-base md:text-lg resize-none min-h-[80px]"
                        required
                    />
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        >
                            {showGuestForm ? (lang === "ar" ? "إغلاق" : "Close") : (lang === "ar" ? "تعريف الهوية" : "Identify")}
                        </button>
                        ...
                        >
                            ...
                            {isSubmitting ? (lang === "ar" ? "جاري..." : "...") : (lang === "ar" ? "نشر" : "Post")}
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div key={cId} className="group relative">
                  <div className="flex gap-6">
                    ...
                    <div className="shrink-0">
                        <div className={`w-14 h-14 rounded-2xl ${getAvatarColor(comment.authorName)} flex items-center justify-center text-white text-xl font-black shadow-lg shadow-black/20 relative z-10`}>
                            {comment.authorName?.[0]?.toUpperCase() || <User />}
                        </div>
                            ...
                            {editingId === cId ? (
                                <div className="space-y-4">
                                    <textarea
                                        ...
                                        className="w-full bg-transparent border-none p-0 text-white focus:outline-none text-lg resize-none min-h-[80px]"
                                        rows="3"
                                        autoFocus
                                    />
